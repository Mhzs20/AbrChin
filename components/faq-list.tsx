"use client";

import { ChevronLeft } from "lucide-react";
import { useState } from "react";

const questions = [
  {
    question: "برای انتخاب باید فنی باشم؟",
    answer: "نه. نیاز و مرحله‌ی پروژه رو می‌گی؛ انتخاب فنی خروجی قطب‌نماست.",
  },
  {
    question: "پرچین دقیقاً چیه؟",
    answer: "پرچین پایه یعنی سرور بعد از کنترل فعال‌بودن و با دسترسی رمزنگاری‌شده‌ی یک‌بارمصرف تحویل می‌شه. پایش پیوسته، بکاپ خودکار و نگه‌داری سیستم‌عامل فعلاً جزوش نیستند مگر جداگانه در سفارش نوشته شوند.",
  },
  {
    question: "بعداً می‌تونم منابع رو بیشتر کنم؟",
    answer: "بله. پیشنهادها برای رشد مرحله‌ای چیده می‌شن تا وقت نیاز ارتقا بدی.",
  },
  {
    question: "وردپرس یا VPS رو کِی انتخاب می‌کنم؟",
    answer: "لازم نیست اول مسیر انتخابش کنی. ابرچین بر اساس نیاز، بستر مناسب رو پیشنهاد می‌ده.",
  },
  {
    question: "قیمت نهایی کی مشخص می‌شه؟",
    answer: "قیمت نهایی هنگام قفل‌شدن پیش‌فاکتور مشخص می‌شود: مبلغ دوره‌ای ماهانه پیش‌پرداخت، به‌همراه مالیات و هزینه پرچین طبق قرارداد همان لحظه. معادل ساعتی/روزانه فقط مرجع نمایشی است و صورتحساب مصرف ساعتی نیست.",
  },
];

export function FaqList() {
  const [openIndex, setOpenIndex] = useState(0);

  return (
    <div className="faq-list">
      {questions.map((item, index) => {
        const open = openIndex === index;
        return (
          <button
            key={item.question}
            className={open ? "open" : ""}
            type="button"
            aria-expanded={open}
            onClick={() => setOpenIndex(open ? -1 : index)}
          >
            <span className="faq-question">{item.question}<ChevronLeft size={18} aria-hidden="true" /></span>
            <span className="faq-answer">{item.answer}</span>
          </button>
        );
      })}
    </div>
  );
}
