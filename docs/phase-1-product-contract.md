...
- پرچین فعال است و Admin می‌تواند مبلغ را صفر/غیرفعال کند:
  - نو / شروع: ۵۰۰٬۰۰۰ تومان در ماه
  - استوار / فعال: ۱٬۵۰۰٬۰۰۰ تومان در ماه
  - کهکشان / پایدار: ۵٬۰۰۰٬۰۰۰ تومان در ماه
...
- پیشنهاد جریان قطب‌نما (برای پیاده‌سازی بعدی):

```text
هدف پروژه → وضعیت فعلی → تحویل مورد انتظار → سطح همراهی پرچین
→ پیشنهاد خدمت + سرور مناسب → بررسی Admin → اجرا و تحویل
```

- سوال‌های پیشنهادی قطب‌نما:
