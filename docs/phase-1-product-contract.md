...
- آروان تنها منبع پشت‌صحنه Admin است؛ Customer هرگز نام
  Provider، تأمین‌کننده یا قیمت پایه آن را نمی‌بیند. برند فروش ابرچین است.
...
- آروان بدون Credential واقعی Fail-closed بماند.
...
- منبع آروان فعال است، اما فقط در Admin دیده می‌شود.
...
- پرچین قرارداد خدمات نسخه‌دار است (شروع / استوار / کهکشان) با Snapshot روی Quote و Order؛ تغییر Admin تعهد سفارش قبلی را عوض نمی‌کند.
...
### ۱۳.۳.۱ منحنی سود و حسابداری عملیاتی

- فروش عادی سرور API-catalog از **منحنی سود پنج‌بازه‌ای** Markup منبع را
  می‌گیرد (۷۰/۶۰/۵۰/۴۰/۳۰٪ هدف با انتقال نرم ریاضی در مرزها). جزئیات:
  `docs/commercial-pricing-v3.md`.
- کف حاشیه پس از تخفیف پیش‌فرض ۲۰٪ است؛ تخفیف بیش از حد بدون اطلاع مشتری
  اعمال نمی‌شود.
- حسابداری عملیاتی (`/admin/accounting`) مدیریتی است، نه دفاتر قانونی.
  جزئیات: `docs/accounting-operations.md`.
...
- برای هر مدل پاسخ، حداقل یک سرور واقعی از فهرست آروان پیشنهاد
...
- Catalog/price/availability sync فقط Read-only است و Provider mutation اجرا
  نمی‌کند.
- `ARVAN_MUTATIONS_ENABLED=false` برای
  Launch حفظ می‌شود؛ ساخت، تغییر و تحویل سرور با Fulfillment دستی و دو
